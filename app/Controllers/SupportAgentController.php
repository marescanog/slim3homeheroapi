...
public function deleteAnouncement(Request $request,Response $response, array $args){
    // -----------------------------------
    // Get Necessary variables and params
    // -----------------------------------
        // Get the bearer token from the Auth header
    ...
    // Check if author, or if manager
        if(!($user_role == 7 || $author == $user_ID)){
            return $this->customResponse->is401Response($response,"Unauthorized Access: Only the author or the manager is allowed to delete this post.");
        }
    ...
    // delete anouncement
        $delete = $this->supportAgent->delete_single_anouncement($a_id);
        // Check for query error
        if($delete['success'] == false){
            // return $this->customResponse->is500Response($response,$delete['data']);
            return $this->customResponse->is500Response($response,"SQLSTATE[42000]: Syntax error or access violation: Please check your query.");
        }

        $resData = [];
        $resData['is_deleted'] =$delete['data'];
        $resData['message'] ="anouncement sucessfully deleted.";
        // $resData['anounce'] = $announce;
        // $resData['author_id'] = $author;
        // $resData['bearerToken'] = $bearer_token;
        // $resData['a_id'] = $a_id;
        // $resData['myRole'] = $user_role;
        return $this->return_server_response($response,"This route works",200,$resData); 
}
